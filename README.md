...
**前瞻性資料限制：**
本專案不抓券商共識 EPS／目標價／法人預估，也不使用 TEJ／CMoney 等付費資料源。
報表中的 EPS 年增率、估算合理價、估算上漲空間與 PEG 替代值，皆由公開歷史資料機械式計算，
淡黃底僅用來提醒「這是估算欄位」，不是券商目標價或投資建議。
...
完成後 Excel 會輸出到 `output/逢低布局選股_日期_時間.xlsx`，含工作表：
...
進階設定可調整 PE 是否必須低於產業基準、融資暴增警示/排除門檻，以及停損緩衝。
...
- 當雲端即時抓取失敗時，自動改用 repo 內的 `seed/snapshot.parquet`、`seed/universe.parquet`，
  介面會顯示「改用內建快照」與 `seed/metadata.json` 記錄的產生日期，app 仍能跑出結果
  （資料為 seed 產生當日）。
...
每次更新 seed 時，程式會同步更新 `seed/metadata.json`，避免部署平台改變檔案時間後誤判資料日期。

> 註：逐檔基本面／法人資料來自 FinMind（`api.finmindtrade.com`），與證交所不同主機，
> 雲端通常可正常連線；上述後備僅針對證交所／櫃買的全市場批次資料。

資料來源層會檢查必要欄位；若 TWSE/TPEX/FinMind 回傳 schema 改變，或本地快取/seed 缺少必要欄位，
會以明確錯誤提示缺少哪些欄位。
...
- **估值**：本益比與同產業中位數比較。
- **融資風險**：近 5 日融資餘額增幅超過 `MARGIN_SURGE_5D` 會列為主要風險；若
  `EXCLUDE_MARGIN_SURGE=True`，則直接排除。
...
MARGIN_SURGE_5D = 0.20         # 近5日融資餘額增幅警示門檻
EXCLUDE_MARGIN_SURGE = False   # True＝融資暴增者排除；False＝只列風險
STOP_BUFFER_BELOW_MID = 0.06   # 停損參考：季線下方 6%
```
...
核心計算與資料流程測試使用 Python 內建 `unittest`，不需額外安裝測試套件：

```bash
python3 -m unittest discover -s tests -v
```

GitHub Actions 會在 push / pull request 時自動安裝依賴、編譯 Python 檔案並執行上述測試。

...
**非目標 / 明確排除**：前瞻性資料（2026/2027 EPS 預估、預估成長率、預估合理價、共識目標價、PEG）需付費資料源，全程以 `需付費資料源`（`config.PAID_PLACEHOLDER`）佔位，不計算、不抓取。輸出皆為機械式計算，非投資建議。
...
- Excel：`output/逢低布局選股_YYYYMMDD.xlsx`。
  ...
  - 主表 15 欄（`excel_report.COLUMNS`）；4 個需付費欄位以淡黃底標記。
...
### P2. 執行速度 — 並行 + 逐檔快取
- **並行深掃**：`ThreadPoolExecutor(max_workers=FINMIND_WORKERS=4)`，`log()` 統一在主緒輸出（Streamlit 安全）。
- **全域節流取代逐次 sleep**：跨緒共用 `FINMIND_MIN_INTERVAL=0.12s` 最小請求間隔，速率與緒數解耦。
- **逐檔當日快取**：同日重跑（含 Streamlit 調參數）近乎即時（實測 30 檔冷跑 ~17s → 熱跑 ~0.6s）。
- 失敗（限流/連線）會拋例外不寫快取，避免卡住整天。
- ⚠️ 免費 token 上限 600 次/小時：首次大範圍掃描仍受此硬限（超量觸發 402/429 → 退避）。建議首跑 `--max 300`，之後靠快取。
